...
public class Main {
     ...
     System.out.println("9.Скачайте цитату из breaking bad и замените все плохие слова звёздочками");
       ...
       int counter = 0;
        for (int i = 0; i < size; i++) {
            ...
            // проверка дубликатов
            boolean wordInArray = isWordInArray(quotes, quote);
            // Удаление повторов
            String notDubles = extractedDuplicates(quotes, i, quote, wordInArray);
            //удаление плохих слов
            String cleanWords = deleteBadWords(notDubles);
    ...
    private static String deleteBadWords(String onlyOneQuote) {
        String [] badWords = new String[]{"Bitch", "bitch", "marijuana", "butts", "fucking", "can go f*ck yourself", "F*ck you", "douchebag", "Douchebags", "Shit"};
        String cleanWords = "";
    ...
    private static String extractedDuplicates(String[] quotes, int i, String notDuble, boolean wordInArray) throws IOException {
        if (!wordInArray) {
            return notDuble;
        } else if (wordInArray) {
            System.out.print("Строка уже есть");
        }
        return "";
    }
